...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstantSend) const
{
	...
	int MASTERNODE_LEVEL = 1000 + floor(chainActive.Height() / 10000) * 500 ;
        if (MASTERNODE_LEVEL > 3000) MASTERNODE_LEVEL = 3000 ;
	int MASTERNODE_LEVEL1 = 1000 ;
	int MASTERNODE_LEVEL2 = 1500 ;
        int MASTERNODE_LEVEL3 = 2000 ;
        int MASTERNODE_LEVEL4 = 2500 ;
        int MASTERNODE_LEVEL5 = 3000 ;
        int MASTERNODE_LEVEL6 = 3500 ;
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == MASTERNODE_LEVEL * COIN);
                } else if(nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fMasterNode) found = pcoin->vout[i].nValue != MASTERNODE_LEVEL * COIN; // do not use Hot MN funds
                } else if(nCoinType == ONLY_1000) {
                    found = (pcoin->vout[i].nValue == MASTERNODE_LEVEL * COIN) || (pcoin->vout[i].nValue == MASTERNODE_LEVEL1 * COIN) || (pcoin->vout[i].nValue == MASTERNODE_LEVEL2 * COIN) || (pcoin->vout[i].nValue == MASTERNODE_LEVEL3 * COIN) || (pcoin->vout[i].nValue == MASTERNODE_LEVEL4 * COIN) || (pcoin->vout[i].nValue == MASTERNODE_LEVEL5 * COIN) || (pcoin->vout[i].nValue == MASTERNODE_LEVEL6 * COIN);
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasterNode && wtx.vout[i].nValue == MASTERNODE_LEVEL * COIN) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterNode && out.tx->vout[out.i].nValue == MASTERNODE_LEVEL * COIN) continue; //masternode input
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 RAN.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 RAN.");
                    ...
                    if (fUseInstantSend) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 RAN."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                        } else {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-rane-address
                        CScript scriptChange;
