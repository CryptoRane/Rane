...
//#3
    ...
    CCoins coins;
        ...
        if (!view.GetCoins(xhash, coins))
            return false;
        mempool.pruneSpent(xhash, coins); 
    LogPrintf("CMasternodeBroadcast::coins height :  %d\n", coins.nHeight);
    LogPrintf("CMasternodeBroadcast::Create success-- %s\n", txin.ToString());

        int MASTERNODE_LEVEL = 1000 + floor(chainActive.Height() / 130000) * 1000 ;
        int MASTERNODE_LEVEL1 = 1000 ;
        int MASTERNODE_LEVEL2 = 2000 ;
        int MASTERNODE_LEVEL3 = 4000 ;
        int MASTERNODE_LEVEL4 = 8000 ;
        int MASTERNODE_LEVEL5 = 12000 ;
        int MASTERNODE_LEVEL6 = 15000 ;
	...
	int cValue =  coins.vout[nOutputIndex].nValue/COIN;
	   ...
	   if (coins.nHeight > 650000 && cValue < MASTERNODE_LEVEL5){
                strErrorRet = strprintf("Masternode TXID has of %d RANE, at block number %d.  %d RANE needed to start masternode now.", cValue, coins.nHeight, MASTERNODE_LEVEL);
                return false;
        }
        if (coins.nHeight > 520000 && cValue < MASTERNODE_LEVEL5){
                strErrorRet = strprintf("Masternode TXID has of %d RANE, at block number %d.  %d RANE needed to start masternode now.", cValue, coins.nHeight, MASTERNODE_LEVEL);
                return false;
        }
        if (coins.nHeight > 390000 && cValue < MASTERNODE_LEVEL4){
                strErrorRet = strprintf("Masternode TXID has of %d RANE, at block number %d.  %d RANE needed to start masternode now..", cValue, coins.nHeight, MASTERNODE_LEVEL);
                return false;
        }
        if (coins.nHeight > 260000 && cValue < MASTERNODE_LEVEL3){
                strErrorRet = strprintf("Masternode TXID has of %d RANE, at block number %d.  %d RANE needed to start masternode now..", cValue, coins.nHeight, MASTERNODE_LEVEL);
                return false;
        }
	
	if (coins.nHeight > 130000 && cValue < MASTERNODE_LEVEL2){
	        strErrorRet = strprintf("Masternode TXID has of %d RANE, at block number %d.  %d RANE needed to start masternode now..", cValue, coins.nHeight, MASTERNODE_LEVEL);
		return false;
	}
...
{
//	if(coins.nHeight > 1000 && coins.vout[0].nValue = MASTERNODE_LEVEL1) return false;
...
//LogPrintf("CMasternodeBroadcast::CheckOutpoint -- Called");
	...
	int MASTERNODE_LEVEL = 1000 + floor(chainActive.Height() / 50000) * 10000 ;
        int MASTERNODE_LEVEL1 = 1000 ;
        int MASTERNODE_LEVEL2 = 2000 ;
        int MASTERNODE_LEVEL3 = 4000 ;
        int MASTERNODE_LEVEL4 = 8000 ;
       ...
       if(coins.vout[vin.prevout.n].nValue !=MASTERNODE_LEVEL * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_LEVEL1 * COIN  && coins.vout[vin.prevout.n].nValue !=MASTERNODE_LEVEL2 * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_LEVEL3 * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_LEVEL4 * COIN && coins.vout[vin.prevout.n].nValue !=MASTERNODE_LEVEL5 * COIN  ) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 RAN, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 RAN tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 RAN tx -> 1 confirmation
