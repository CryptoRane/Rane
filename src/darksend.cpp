...
{
    if(fLiteMode) return; // ignore all Rane related functionality
...
/*
    ...
    allow endless transaction that would bloat Rane and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if((out.nValue == MASTERNODE_LEVEL * COIN || out.nValue == MASTERNODE_LEVEL1 * COIN || out.nValue == MASTERNODE_LEVEL2 * COIN || out.nValue == MASTERNODE_LEVEL3 * COIN || out.nValue == MASTERNODE_LEVEL4 * COIN || out.nValue == MASTERNODE_LEVEL5 * COIN || out.nValue == MASTERNODE_LEVEL6 * COIN)  && out.scriptPubKey == payee) return true;
    }
...
{
    if(fLiteMode) return; // disable all Rane specific functionality
