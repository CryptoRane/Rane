 ...
 *
 *   // network
 *   CRaneNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.rane.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CRaneNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: RaneNetwork
// //   -- signature requirements : Key1(User)
// class CRaneNetworkVariable : public CGovernanceObject
// {
// private:
