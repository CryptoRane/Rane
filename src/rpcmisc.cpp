...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total rane balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized rane balance of the wallet\n"
    ...
    obj.push_back(Pair("masternode price", MASTERNODE_LEVEL));
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "rane|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "rane")
            + HelpExampleRpc("debug", "rane,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"raneaddress\"\n"
            "\nReturn information about the given rane address.\n"
            "\nArguments:\n"
            "1. \"raneaddress\"     (string, required) The rane address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"raneaddress\", (string) The rane address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Rane address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are rane addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) rane address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"raneaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"raneaddress\"  (string, required) The rane address to use for the signature.\n"
