...
const char * const BITCOIN_CONF_FILENAME = "rane.conf";
const char * const BITCOIN_PID_FILENAME = "raned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rane" is a composite category enabling all Rane-related debug output
            if(ptrCategory->count(string("rane"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RaneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RaneCore
    // Mac: ~/Library/Application Support/RaneCore
    // Unix: ~/.ranecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RaneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RaneCore";
#else
    // Unix
    return pathRet / ".ranecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rane.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rane.conf
